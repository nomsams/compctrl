...
export type StreamMessage = { type: 'stream'; action: 'request' };
export type SystemMessage = { type: 'system'; action: 'restart' | 'shutdown' };
export type PingMessage = { type: 'ping'; sentAt: number };
export type AuthResponseMessage = { type: 'auth-response'; challenge: string; nonce: string; proof: string };
...
export type HostMessage =
  ...
  | { type: 'auth-ok' }
  | { type: 'ready'; computerName: string; jigglerEnabled: boolean; screenBlanked: boolean; dictationAvailable: boolean }
